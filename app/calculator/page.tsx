...
function CalculatorPage() {
  ...
  }, [quiltWidth, quiltLength, quiltingType, threadChoice, battingChoice, battingLengthAddition, clientSuppliesBatting, bindingType, bobbinCount, settings]);
  ...
  const calculateTotals = () => {
    ...
    if (quiltingType && area > 0) {
      const rate = settings.pricingRates?.[quiltingType as keyof typeof settings.pricingRates] || 0;
      quilting = area * rate;
    }

    // Thread calculation
    if (threadChoice) {
      const threadOption = settings.threadOptions?.find(t => t.name === threadChoice);
      thread = threadOption?.price || 0;
    }

    // Batting calculation
    if (battingChoice && !clientSuppliesBatting) {
      const battingOption = settings.battingOptions?.find(b => b.name === battingChoice);
    ...
    if (bindingType && w > 0 && h > 0) {
      ...
      if (bindingType === 'Top Attached Only') {
        binding = perimeter * (settings.pricingRates?.bindingTopAttached || 0.10);
      } else if (bindingType === 'Fully Attached') {
        binding = perimeter * (settings.pricingRates?.bindingFullyAttached || 0.20);
      }
    }
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-muted mt-1">Create estimate for new project</p>
          </div>
          <button
            onClick={() => router.push('/')}
        ...
        <div className="bg-white border border-line rounded-card p-4 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <div className="font-bold text-sm">Tier: {isPaidTier ? 'PAID' : 'FREE'}</div>
              <div className="text-xs text-muted mt-1">
                {isPaidTier ? 'Settings auto-populate' : 'Manual entry required'}
              </div>
            </div>
            <button
          ...
          {!isPaidTier && (
            <div className="mt-3 p-3 bg-gold/10 rounded-xl text-xs">
              ...
              <div>You'll manually enter pricing for each estimate. Upgrade to PAID to save rates and auto-populate.</div>
            </div>
          )}
        </div>
        ...
        <div className="bg-white border border-line rounded-card p-6">
          ...
          <div className="mb-6">
            ...
            {quiltWidth && quiltLength && (
              <div className="mt-2 text-xs text-muted">
                Area: {(parseFloat(quiltWidth) * parseFloat(quiltLength)).toFixed(0)} sq in
              </div>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            {isPaidTier && settings?.pricingRates ? (
              ...
              >
                ...
                <option value="lightE2E">Light Edge-to-Edge (${settings.pricingRates.lightE2E || 0}/sq in)</option>
                <option value="standardE2E">Standard Edge-to-Edge (${settings.pricingRates.standardE2E || 0}/sq in)</option>
                <option value="lightCustom">Light Custom (${settings.pricingRates.lightCustom || 0}/sq in)</option>
                <option value="custom">Custom (${settings.pricingRates.custom || 0}/sq in)</option>
                <option value="denseCustom">Dense Custom (${settings.pricingRates.denseCustom || 0}/sq in)</option>
              </select>
            ) : (
              <input
                type="text"
          ...
          <div className="mb-6">
            ...
            {isPaidTier && settings?.threadOptions && settings.threadOptions.length > 0 ? (
              <select
                value={threadChoice}
          ...
          <div className="mb-6">
            <label className="flex items-center gap-3">
              ...
              />
              <div>
                <div className="text-sm font-bold text-plum">Client Supplies Own Batting</div>
                <div className="text-xs text-muted">Batting cost will be $0</div>
              </div>
            </label>
          </div>
          ...
          {!clientSuppliesBatting && (
            <>
              <div className="mb-6">
                ...
                {isPaidTier && settings?.battingOptions && settings.battingOptions.length > 0 ? (
                  <select
                    value={battingChoice}
          ...
          <div className="mb-6">
            ...
            >
              ...
              <option value="Top Attached Only">Top Attached Only (${settings?.pricingRates?.bindingTopAttached || 0.10}/in)</option>
              <option value="Fully Attached">Fully Attached (${settings?.pricingRates?.bindingFullyAttached || 0.20}/in)</option>
            </select>
          </div>
          ...
          <div className="border-t border-line pt-6 mt-6">
            ...
            <div className="border-t border-line pt-3 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-muted">Tax ({settings?.taxRate || 0}%)</span>
